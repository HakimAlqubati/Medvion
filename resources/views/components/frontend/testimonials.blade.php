...
<section id="testimonials" class="py-20 md:py-32 {{ $altBg ? 'bg-gray-50' : 'bg-white' }} relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 reveal delay-100">
            @foreach($testimonials as $testimonial)
            <div class="h-auto">
                <div class="bg-white rounded-3xl p-8 md:p-10 shadow-sm border border-gray-100 h-full flex flex-col transition-all duration-300 hover:shadow-lg hover:border-primary/20 group">
                    ...
                    <div class="flex items-center gap-4 mt-auto pt-6 border-t border-gray-50">
                        <div class="relative">
                            ...
                            <img src="{{ asset($testimonial->avatar_image) }}" alt="{{ $testimonial->client_name['ar'] }}" class="w-14 h-14 rounded-full object-cover shadow-sm ring-2 ring-primary/10" loading="lazy" decoding="async">
                            @else
                        ...
                        <div>
                            <h4 class="text-base font-bold text-gray-900">{{ $testimonial->client_name['ar'] }}</h4>
                            <p class="text-sm text-primary font-medium">{{ $testimonial->role['ar'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
